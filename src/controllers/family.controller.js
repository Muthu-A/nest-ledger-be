...
const inviteTemplate = require("../services/inviteTemplate");
...
exports.createFamily = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, error: "Server error", code: "SERVER_ERROR" });
  }
};
...
exports.getFamily = async (req, res) => {
  try {
    ...
    if (!family)
      return res
        .status(404)
        .json({ success: false, error: "Family not found", code: "NOT_FOUND" });
    ...
    const recentActivities = [...inviteActivities, ...joinedActivities].sort(
      (a, b) => {
    ...
    return res.json({
      success: true,
      data: { family, members, recentActivities },
    });
  } catch (err) {
    console.error(err);
    return res
  ...
  try {
    ...
    if (!membership || membership.role !== "owner")
      return res
        ...
        .json({
          success: false,
          error: "Only owner can delete family",
          code: "FORBIDDEN",
        });
    // clear users
    const members = await FamilyMember.find({ familyId });
  ...
  try {
    ...
    try {
      await emailService.sendEmail({
        ...
        subject: "You've been invited to NestLedger",
        html: inviteTemplate({
          inviterName: req.user.name,
          inviteCode:code,
          inviteLink: `${process.env.FRONTEND_URL}/login`,
        }),
      });
      console.log("Invite email sent");
      
    } catch (e) {
      console.log("Failed to send invitiation email");
    }
    ...
    return res
      ...
      .json({
        success: true,
        data: { invitationCode: code, expiresAt, link },
...
exports.acceptInvitation = async (req, res) => {
  try {
    ...
    const invitation = await Invitation.findOne({
      invitationCode: { $regex: `^${esc}$`, $options: "i" },
    });
    if (!invitation)
      return res
        .status(400)
    ...
    if (invitation.email.toLowerCase() !== req.user.email.toLowerCase())
      return res
        ...
        .json({
          success: false,
          error: "Invitation not for your account",
          code: "FORBIDDEN",
        });
    ...
    // Notify the joining user (if connected) and add their sockets to the family room
    try {
      const socketService = require('../services/socketService');
      console.log('[acceptInvitation] Triggering socket events for userId:', req.user._id, 'familyId:', invitation.familyId);
      
      // Get joining user's sockets (global tracking)
      const userSockets = socketService.getSocketsByUser(req.user._id);
      console.log('[acceptInvitation] User sockets count:', userSockets.length, 'socketIds:', userSockets);
      
      socketService.emitToUserSockets(req.user._id, 'member-joined', { familyId: invitation.familyId, role: 'editor', message: 'You joined the family' });
      console.log('[acceptInvitation] Emitted member-joined to user');
      
      socketService.addUserSocketsToFamily(invitation.familyId, req.user._id);
      console.log('[acceptInvitation] Added user sockets to family room');
      
      // broadcast updated online members to family with full details
      const members = await socketService.getOnlineMembersWithDetails(invitation.familyId);
      console.log('[acceptInvitation] Online members with details:', members);
      socketService.emitToFamily(invitation.familyId, 'family-online-members', members);
      
      // Broadcast join notification to entire family
      socketService.emitToFamily(invitation.familyId, 'user-joined-family', { 
        familyId: invitation.familyId,
        userId: req.user._id, 
        name: req.user.name, 
        email: req.user.email,
        message: `${req.user.name} has joined the family`,
        timestamp: new Date()
      });
      console.log('[acceptInvitation] Emitted user-joined-family to family');
    } catch (notifyErr) {
      console.error('notify joined member error', notifyErr);
    }
    await invitation.deleteOne();
    return res.json({
      success: true,
      data: { familyId: invitation.familyId, role: "editor" },
...
exports.listMembers = async (req, res) => {
  try {
    ...
    const members = await FamilyMember.find({ familyId }).populate(
      "userId",
      "name email",
    );
...
exports.removeMember = async (req, res) => {
  try {
    ...
    const requester = await FamilyMember.findOne({
      familyId,
      userId: req.user._id,
    });
    ...
    // notify removed user via socket and remove their sockets from family room
    try {
      const socketService = require('../services/socketService');
      // Send targeted event to any sockets the user currently has (global tracking)
      socketService.emitToUserSockets(userId, 'member-removed', { familyId, message: 'You were removed from the family' });
      // Remove user's sockets from family tracking/room if present
      socketService.removeUserFromFamily(familyId, userId);
      // Broadcast updated online members with details
      ...
      console.error('notify removed member error', notifyErr);
    }
...
exports.leaveFamily = async (req, res) => {
  try {
    ...
    if (membership.role === "owner") {
      const ownerCount = await FamilyMember.countDocuments({
        familyId,
        role: "owner",
      });
      if (ownerCount <= 1)
        return res
          ...
          .json({
            success: false,
            error: "Cannot leave family with no owner",
            code: "NO_OWNER",
          });
    }
